...
* 描述:
*
* 为平衡二叉树的使用提供通用接口，不区分二叉树节点
* 的数据类型。数据类型可以任意，用户只需要定义节点
* 大小的比较函数和节点的释放函数即可。
* 对于 Vxworks操作系统还可以提供二叉树互斥的函数。
* 本通用有序平衡二叉树可以实现自平衡功能，无论在添
* 加还是删除节点之后，此树都处于平衡状态。
*
*/ 
#include "avlTree.h"
...
tAVLTree *avlTreeCreate
	(
	PkeyCompareFunc keyCompareFunc,
	PfreeFunc freeFunc
	)
{
	...
	if(pTree != (tAVLTree *)0)
	{
		...
		pTree->keyCompare = keyCompareFunc;
		pTree->free = freeFunc;
#ifdef ORDER_LIST_WANTED
		pTree->pListHeader = pTree->pListTail = AVL_NULL;
#endif
...
int avlTreeUpdate
	(
	tAVLTree *pTree ,
	TREE_NODE *pUpdateNode
	)
{
	int ret = 0;
	TREE_NODE * pFindNode = NULL;
	pFindNode = avlTreeFind( pTree, pUpdateNode);
	if ( pFindNode != AVL_NULL)
	{
		*pFindNode = *pUpdateNode;
		ret = 1;
	}
	return ret;
}
